...
| **Highlights tab + in-app seek** — Highlights tab lists scoring moments and stream replay flags (auto + manual); tap seeks the match hub stream player (not external YouTube) via `match_stream_seek_provider.dart` | Done — `match_highlights_list.dart`, `match_stream_watch_section.dart` |
