...
| Item | Status |
|------|--------|
| **Match scorecard live stream player** — pinned below app bar; **Live on / Live off** toggle; multi-session **stream picker** (bottom sheet when 2+ sessions); labels from each entry's `addedAt`/`endedAt`; stream metadata preserved across scoring merges + local cache sync on go-live; hub restores player when returning from child routes | Done — `match_stream_watch_section.dart`, `match_update_merge.dart`, `match_repository.dart` |
...
| **Replay marker pre-roll** — auto AND manual markers roll back **30s** before the scored moment (ball event timestamp when available) so viewers see the delivery, not post-confirm; clamped to ≥0 | Done — `_kReplayPreRollMs` in `streaming_dashboard_screen.dart` |
| **Highlights tab + in-app seek** — Highlights tab lists scoring moments and stream replay flags (auto + manual); newest-first by ball `sequence` (stable during live/offline); tap seeks the match hub stream player (switches to parent session URL + offset; works across manual/auto YouTube, Facebook, multi-session) via `match_stream_seek_provider.dart` | Done — `match_highlights_list.dart`, `match_highlights_merger.dart`, `match_stream_watch_section.dart`, `match_stream_playback.dart` |
...
| **Match introduction overlay** — once per live session after RTMP connects: 10s full-screen TV-style intro (captains, teams, match type, overs, tournament/venue, CrickFlow LIVE branding) with staggered broadcast animations, then scorebug; compositor orchestration only (no burn-in pipeline changes) | Done |
| **Opening batsmen overlay** — after match intro, 5s pair overlay with two batter intro career cards side by side (striker + non-striker from current innings); then opening bowler or scorebug; once per live session | Done |
| **Opening bowler overlay** — after opening batsmen (or match intro if no pair), 5s first-bowler career card in normal bowler intro position with scorebug visible; then normal scorebug; once per live session | Done |
| **Innings break presentation** — 10 centered broadcast cards (15s) with scorebug title/LIVE header only; large target panel (overs + balls) on side; 1st-innings recap left; enlarged readable typography | Done |
