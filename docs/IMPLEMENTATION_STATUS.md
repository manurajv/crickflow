...
| **Highlights tab + in-app seek** — Highlights tab lists scoring moments and stream replay flags (auto + manual); tap seeks the match hub stream player (switches to parent session URL + offset; works across manual/auto YouTube, Facebook, multi-session) via `match_stream_seek_provider.dart` | Done — `match_highlights_list.dart`, `match_stream_watch_section.dart`, `match_stream_playback.dart` |
