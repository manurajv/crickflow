  ...
  try {
    ...
    return cameras.map((Map<dynamic, dynamic> camera) {
      return CameraDescription(
        ...
        focalLengthMm: (camera['focalLength'] as num?)?.toDouble(),
      );
    }).toList();
...
class CameraDescription {
  ...
  /// Shortest available focal length in mm (Android back lenses), when reported.
...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  /// Switch to a physical camera by Android camera ID (e.g. "0", "2", "3").
  Future<void> switchCameraById(String cameraName) async {
    if (!value.isInitialized! || _isDisposed) {
      throw CameraException(
        'Uninitialized CameraController',
        'switchCameraById called on uninitialized controller',
      );
    }
    try {
      await _channel.invokeMethod<void>(
        'switchCameraById',
        <String, dynamic>{'cameraName': cameraName},
      );
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  ...
  /// Digital zoom level (1.0 = no zoom).
  ...
  /// Maximum supported digital zoom on the active camera.
  Future<double> getMaxZoom() async {
    if (!value.isInitialized! || _isDisposed) return 1.0;
    try {
      final max = await _channel.invokeMethod<double>('getMaxZoom');
      return max ?? 1.0;
  ...
  /// Toggle front / back camera using the native pedro switch.
  ...
  /// Exposure compensation in EV steps (-2 … 2 typical).
  Future<void> setExposureCompensation(double ev) async {
    if (!value.isInitialized! || _isDisposed) return;
    try {
      await _channel.invokeMethod<void>(
        'setExposureCompensation',
        <String, dynamic>{'ev': ev},
      );
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  ...
  /// Lock or unlock auto-focus on the active camera.
  Future<void> setFocusLock(bool locked) async {
    if (!value.isInitialized! || _isDisposed) return;
    try {
      await _channel.invokeMethod<void>(
        'setFocusLock',
        <String, dynamic>{'locked': locked},
      );
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  ...
  /// Tap-to-focus at normalized preview coordinates.
  ...
  /// Burns a transparent PNG overlay into the outgoing RTMP video (Android).
  Future<void> updateStreamOverlay(
    Uint8List png, {
    required int width,
    required int height,
  }) async {
  ...
  /// Removes the RTMP overlay burn-in filter.
  Future<void> clearStreamOverlay() async {
    if (!value.isInitialized! || _isDisposed) return;
    if (!Platform.isAndroid) return;
    try {
      await _channel.invokeMethod<void>('clearStreamOverlay');
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  }
